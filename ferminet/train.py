...
def train(cfg: ml_collections.ConfigDict, writer_manager=None):
  ...
  if cfg.optim.get('spin_energy', 0.0) > 0.0:
    ...
    def local_energy_and_kullback_fn(params, keys, data):
      ...
      # weight = 0.1
      # local_energy_and_kullback = (1 - weight) * local_energy + weight * local_kullback_fn(params, data, None)
      local_energy_and_kullback = local_kullback_fn(params, data, None)
      return local_energy_and_kullback, aux_data
      # return local_energy, aux_data
      ...
      if t % cfg.log.stats_frequency == 0:
        ...
        for key in observable_data:
          ...
          if key == 'dipole':
            ...
            obs_data_list = obs_data.reshape(-1)
            writer_kwargs[key] = ",".join(map(lambda x: f"{x}", obs_data_list))
            logging_str += ', <S_rho>=' + '%03.4f,' * obs_data_list.shape[0]
            logging_args += *obs_data_list,
